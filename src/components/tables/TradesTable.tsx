...
import { Link as L, SquareArrowOutUpRight } from "lucide-react";
  ...
  return (
    <div>
      ...
      <table className="table">
        <thead>
          <tr
            className="bg-border/30 *:text-sm *:text-muted-foreground *:py-4 *:px-4"
          >
            <td>TICKET</td>
            <td>TYPE</td>
            <td>DATE</td>
        ...
        <tbody className="divide-y border-b">
          {filteredTrades.map((trade) => (
            <tr
              key={trade.id}
              className="has-hover:bg-border/20 *:py-4 *:px-3"
            >
              <td>{trade.ticket}</td>
              <td>{trade.type}</td>
